...
public class CompletionProposal implements ICompletionProposal {
	...
	public static Collection<CompletionProposal> createProposals(Iterable<String> proposals, 
			CompletionContext context, ICompletionFilter filter) {
		Collection<CompletionProposal> result = new Vector<CompletionProposal>();
		for (String proposal : proposals) {			
			if (proposal != null && proposal.startsWith(context.getIdentifierPrefix())) {
				if (filter == null || filter.accept(proposal)) {
					result.add(new CompletionProposal(proposal, proposal.substring(
	...
	public CompletionProposal(final String displayText, String documentText, int documentOffset) {
		super();
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		final CompletionProposal other = (CompletionProposal) obj;
