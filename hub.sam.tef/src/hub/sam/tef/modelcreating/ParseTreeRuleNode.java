...
/**
 * Traverse the syntax tree and build a model based on the grammar
 * rules, annotations and statements. Model construction happens in
 * two passes: first the model elements are created by a recursive
 * execution of <code>{@link #createModel(IModelCreatingContext, Object)}</code>
 * then references are resolved and non-local model changes are performed
 * by a recursive execution of <code>{@link #resolveModel(IModelCreatingContext, ResolutionState)}</code>
 * 
...
public final class ParseTreeRuleNode extends ParseTreeNode {
	...
	/**
	 * traverse the abstract syntax tree and build the model
	 * based on the grammar rule annotations
	 * 
	 * @param context stores the current knowledge about the model creation
	 * @param parentNodeValue value created at the parent node
	...
	public Object createModel(IModelCreatingContext context, Object parentNodeValue) 
		...
		if (valueBinding != null) {
			// the node has a value binding, create a new value based on the binding
		...
		if (!(fRule instanceof SimpleRule))
			throw new ModelCreatingException("ERROR: trying to create a model based on a rule with no right-hand side: "+fRule);

		SimpleRule ruleRhs = (SimpleRule)fRule;

		//System.err.println("rule size: "+ruleRhs.getRhs().size()+", array size: "+)
		...
		loop: for (int currentRhsPart=0; currentRhsPart<ruleRhs.getRhs().size(); currentRhsPart++)
		{
			...
			if (propertyBinding instanceof ReferenceBinding) {				
				...
				childValues[currentRhsPart] = childNode.createModel(context, myNodeValue);
						...
						propertyBinding instanceof CompositeBinding)
				{
					...
					context.getSemanticsProvider().getPropertyCreationSemantics(compositeBinding).addValue(
							childNode, myNodeValue, childValues[currentRhsPart], context, compositeBinding);
					// the value is bound
		...
		for (int valueIndex=0; valueIndex < childValues.length; valueIndex++)
		{
			if (childValues[valueIndex] != null && !childValuesUsed[valueIndex])
			{
				if (unboundValue == null) {	// child node value is unset
					...
					errorMsg +=
						"\n"+"Rule: "+fRule+"\n"+
						"has value: "+unboundValue+"\n"+
						"and gets new value at node #"+valueIndex+": "+childValues[valueIndex];
		...
		if (valueBinding != null) {
			...
			if (unboundIndex >= 0) { // this block is for error diagnosis only
				...
				errorMsg +=
					...
					"Unbound value at node #"+unboundValue+": "+childValues[unboundIndex];
				//throw new ModelCreatingException(errorMsg);
				System.err.println(errorMsg);
			}
		} else {
	 ...
	 * 
	 * @param context stores the available knowledge on the creation of the model
	 * @param aResolutionState stores the most recent candidate for
	 ...
	 */
	@Override
	public Object resolveModel(IModelCreatingContext context, ResolutionState aResolutionState)
			throws ModelCreatingException {
		...
		if (valueBinding != null) {
			...
			// for element reference bindings, the loose actual is the value of the rule's
			// LHS node, set this value explicitly (this should be consistent with any
			// surrounding composite binding
			if (value instanceof EObject && valueBinding instanceof ElementReferenceBinding)
				resolutionState.setLooseActual((EObject)value);
			if (valueBinding instanceof PrimitiveBinding || valueBinding instanceof ConstantBinding)
		...
		loop: for (int currentRhsPart=0; currentRhsPart<ruleRhs.getRhs().size(); currentRhsPart++)
		{
			...
			if (propertyBinding != null) {
				if (propertyBinding instanceof CompositeBinding) {
					...
					try {
						...
						// in #createModel(), so do nothing here, is this correct?
					} catch (NullPointerException npe) {
						System.err.println("error resolving "+propertyBinding+" in rule "+fRule+ " at "+rhsPart+ " in "+resolutionState);
						context.addError(new hub.sam.tef.semantics.Error(this.getPosition(), npe.getLocalizedMessage()));
					}
					...
					try {
						...
						System.err.println("error resolving "+propertyBinding+" in rule "+fRule+ " at "+rhsPart);
						context.addError(new hub.sam.tef.semantics.Error(this.getPosition(), mce.getLocalizedMessage()));
					}
				} else {
				...
				childValues[currentRhsPart] = childNode.resolveModel(context, resolutionState);
			}		
				...
				&& rhsPart.getActionBinding().getStatements() != null)
			{
				...
				{
					EList<Statement> statements = rhsPart.getActionBinding().getStatements();
					for (Statement st : statements) {
						...
						for (Integer parameterIndex : st.getMethodParameters()) {
							if (parameterIndex == -1)
								methodParameters.add(this.value);
							else
								methodParameters.add(childValues[parameterIndex]);
						}
						// parameters populated, call evaluation method
						context.evaluateActionStatement(st.getMethodName(), methodParameters);
					}
				}
			}
		}
		...
		// needs to be resolved in the model, this can only be done here, after all
		// nodes of the rule have been evaluated
		if (valueBinding != null && valueBinding instanceof ElementReferenceBinding) {
			ElementReferenceBinding erBinding = (ElementReferenceBinding) valueBinding;
			try {
				// is it feasible to use the object to resolve as its own context?
				...
				EObject referencedObject = context.getSemanticsProvider().
						...
						resolve(this, contextObject, objectToResolve, context, erBinding.getMetaclass());
				...
				System.err.println("error resolving "+erBinding+" in rule "+fRule+ " at "+fRule.getLhs());
				context.addError(new hub.sam.tef.semantics.Error(this.getPosition(), mce.getLocalizedMessage()));
			}
		}
	 ...
	 * @param currentRhsPart index of the current rhs part (position in the rule)
				...
				for (Integer parameterIndex : st.getMethodParameters()) {
					if (parameterIndex > currentRhsPart) {
						...
						if (parameterIndex >= ruleRhs.getRhs().size())
							...
							symbolName = "symbol '"+ruleRhs.getRhs().get(parameterIndex)+"'";
						
						String errorMsg = "Tried to access undefined Parse Tree Node value of "+symbolName+" at symbol '"+rhsPart+"' in rule: "+fRule;
						errorMsg += "\n"+"skipping all method calls at symbol '"+rhsPart+"', starting at call '"+st.getMethodName()+"'";
						throw new ModelCreatingException(errorMsg);
					}
