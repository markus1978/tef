...
/**
 * A value check semantics that triggers the validation frameworks. This is also
 * used as the default value check semantics for all model elements.
 */
		...
		if (value.eContainer() == null) {
			...
			if (!status.isOK()) {				
				if (!status.isMultiStatus()) {
					...
					context.addError(new ModelCheckError(cstatus.getMessage(), 
							cstatus.getTarget()));
				} else {
